...
/**
 * Product media asset. An image, or a video when the video field is set.
 */
export interface ProductBusMedia {
  ...
  roles?: string[];
  /** Absolute URL of an associated video asset, when this media entry represents a video. */
  video?: string;
}
...
export interface ProductBusEntry {
  ...
  jsonldExtensions?: Record<string, unknown>;
  /**
   * Shipping options, as a packed string, a single option object, or an array of option objects.
...
/**
 * Server-calculated merchandise discount allocated to an order line.
 */
export interface OrderItemDiscount {
  /** Applied discount or promotion identifier. */
  id: string;
  /** Positive fixed-scale order-currency amount discounted from the line. */
  amount: string;
}

/**
 * A server-resolved bundle component returned on a stored order.
 */
export interface StoredNestedBundleItem {
  /** Display name of the component. */
  name?: string;
  /** SKU of the component. */
  sku?: string;
  ...
  /** Price information for a product or variant. */
  price: ProductBusPrice;
  /** Server-calculated allocations for this component. For a bundle, component allocations are a non-additive decomposition of the parent allocation: matching discount IDs sum to the parent value. Component-line consumers calculate `price.final * quantity - sum(discounts.amount)` here and must not also subtract the parent allocation. */
...
/**
 * A server-resolved order line returned from order preview or storage.
 */
export interface StoredOrderItem {
  ...
  /** Option values selected by the customer. */
  selectedOptions?: SelectedOption[];
  /** Server-resolved components with non-additive allocations that replace parent allocations for component-line consumers. */
  bundleItems?: StoredNestedBundleItem[];
  /** Arbitrary custom data for this line item. */
  custom?: Record<string, unknown>;
  /** Server-calculated allocations for this line. For a bundle parent, component allocations are a non-additive decomposition of these values: matching discount IDs sum to this parent allocation. Parent-line consumers calculate `price.final * quantity - sum(discounts.amount)` here; component-line consumers use component allocations instead. Never subtract both levels. */
  discounts?: OrderItemDiscount[];
}
